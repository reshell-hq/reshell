...
/**
 * The pomodoro timer state machine (CONTEXT: "Focus split" / "Focus countdown").
 * Pure transitions ported from the pre-rewrite `src/internal-tools/pomodoro.ts`:
 * focus-split presets plus a custom split, single-interval countdown, and
 * reload-persistent end timestamps. Chime is off by default.
 */
...
export function resolveFocusSplit(
  ...
  return (
    BUILTIN_FOCUS_SPLITS.find((split) => split.id === splitId) ??
    BUILTIN_FOCUS_SPLITS[0]
  );
}
